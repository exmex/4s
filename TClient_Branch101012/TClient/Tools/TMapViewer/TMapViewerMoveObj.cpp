...
CTMapViewerMoveObj::CTMapViewerMoveObj()
{
	m_bDrawShadow = TRUE;
	m_bDrawName = TRUE;
}
...
void CTMapViewerMoveObj::ReleaseData()
{
...
void CTMapViewerMoveObj::CalcDIR( DWORD dwTick,
								 BYTE bMouseDIR,
								 BYTE bKeyDIR)
{
...
void CTMapViewerMoveObj::CalcDIR( WORD wMoveDIR, WORD wDIR)
{
	...
	if( m_wMoveDIR != wMoveDIR )
		if( wMoveDIR != MOVE_NONE )
		{
			...
			for( int i=0; i<6; i++)
				SetPIVOT( dwPivotID[i][0], 0.0f, fRotUP * FLOAT(dwPivotID[i][1]), 0.0f);
		}
		else
		{
			for( int i=0; i<6; i++)
				SetPIVOT( dwPivotID[i][0], 0.0f, 0.0f, 0.0f);
		}
...
D3DXMATRIX CTMapViewerMoveObj::GetPITCH( WORD wDIR)
{
...
BYTE CTMapViewerMoveObj::Fall( LPD3DXVECTOR2 pFallDIR)
{
	if( IsFall() || m_vTACTION[TA_FALL]->m_bLevel < m_bACTLevel )
		return FALSE;
	...
	m_bACTLevel = m_vTACTION[TA_FALL]->m_bLevel;
...
BYTE CTMapViewerMoveObj::Jump()
{
	if( IsJump() || m_vTACTION[TA_JUMP]->m_bLevel < m_bACTLevel )
		return FALSE;
	...
	m_bACTLevel = m_vTACTION[TA_JUMP]->m_bLevel;
	SetAction( vActionID.m_dwActID, vActionID.m_dwAniID);
...
void CTMapViewerMoveObj::CalcTick( LPD3DXVECTOR3 pPREV,
								  CD3DDevice *pDevice,
								  CTachyonRes *pRES,
								  DWORD dwTick)
{
...
void CTMapViewerMoveObj::CalcHeight( LPD3DXVECTOR3 pPREV,
								    CTMapViewerMAP *pMAP,
									DWORD dwTick)
{
	...
	if(IsFall())
	{
		...
		FLOAT fFallHeight = m_fFallHeight - CTMapViewerMAP::m_fGravity * fTime * fTime / 2.0f;
	...
	else if(IsJump())
	{
		...
		FLOAT fJumpHeight = m_fJumpHeight + m_fJumpSpeed * fTime - CTMapViewerMAP::m_fGravity * fTime * fTime / 2.0f;

		if( !m_bSwim || fTime < m_fJumpSpeed / CTMapViewerMAP::m_fGravity || fJumpHeight > fWaterHeight )
		{
	...
	m_bLand = fabs(fLandHeight - fHeight) > 0.01f ? FALSE : TRUE;
	...
	if( fHeight < m_vPosition._42 && !IsFall() && !IsJump() && (
		fMove == 0.0f || CTMapViewerMAP::m_fFallBound < (m_vPosition._42 - fHeight) / fMove) )
		Fall(&vMOVE);
	...
	else if( dwTick && m_vPosition._42 <= pPREV->y && (m_vPosition._42 <= fHeight || bLAND) )
	{
		...
		if( IsJump() || IsFall() )
		{
			...
			m_bACTLevel = m_vTACTION[m_bAction]->m_bLevel;
		}
		else
			m_bAction = GetTAction();
	}
...
BYTE CTMapViewerMoveObj::GetTAction()
{
	switch(m_bAction)
	{
	case TA_DEAD	:
...
BYTE CTMapViewerMoveObj::GetTAction( BYTE bMouseDIR)
{
	switch(m_bAction)
	{
	case TA_DEAD		:
	case TA_DIE			: return TA_DEAD;
	default				:
		if(m_vTACTION[m_bAction]->m_bContinue)
			return m_bAction;
...
void CTMapViewerMoveObj::CancelTAction()
{
	BYTE bAction = GetTAction(m_bMouseDIR);

	if(m_vTACTION[bAction]->m_bCancel)
		m_bAction = GetTAction();

	ClearTActionSFX(&m_vTACTIONSFX);
}

void CTMapViewerMoveObj::RemoveTAction()
{
	BYTE bAction = GetTAction(m_bMouseDIR);

	if(m_vTACTION[bAction]->m_bContinue)
		m_bAction = GetTAction();

	ClearTActionSFX(&m_vTACTIONSFX);
}

void CTMapViewerMoveObj::SetAction( DWORD dwActionID,
								   DWORD dwAniID)
{
	CTMapViewerObjBase::SetAction(
		dwActionID,
		dwAniID);
}

void CTMapViewerMoveObj::ResetAction( CD3DDevice *pDevice,
									 CTachyonRes *pRES)
{
	if( m_bACTLevel > m_vTACTION[m_bAction]->m_bLevel )
		return;
...
WORD CTMapViewerMoveObj::GetTargetDIR( FLOAT fTargetX,
									  FLOAT fTargetZ)
{
	if( fTargetZ != 0.0f )
	{
