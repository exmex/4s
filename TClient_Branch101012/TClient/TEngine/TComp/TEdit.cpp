...
HRESULT TEdit::DrawText()
{
	...
	int nFrom = 0;
	// Arab Ver.
	BOOL bFLIP_SCREEN = (m_nCountryFlag == COUNTRY_FLAG_EGYPT) ? TRUE : FALSE;
	if( m_bConverting )
	...
	if(m_strDisplayText.GetLength() > 0)
	{
		...
		while(bContinue)
		{
			...
			// Arab Ver.
			int nPosition = nCaret - nFrom;
			TextToPoint(strDrawBuf, nPosition, ptCaret, bFLIP_SCREEN);
			TextToPoint(strDrawBuf, 0, ptText, FALSE);
			// Arab Ver.
			if(m_nCountryFlag == COUNTRY_FLAG_EGYPT)
			{
				CPoint ptFirstPos;
				TextToPoint(strDrawBuf, nCaret - nFrom, ptFirstPos, FALSE);
				nPosition =  ptFirstPos.x;
			}
			else
				nPosition = ptCaret.x;
			// Arab Ver.
			if( nFrom < 0 ||
				m_nTextExtent > abs(nPosition - ptText.x)|| 
				nFrom >= m_strDisplayText.GetLength()) 
				bContinue = FALSE;
			else
			{
	...
	if( m_pCaret && !m_blIsLocked )
	{
		...
		if(strDrawBuf.IsEmpty())
			TextToPoint(strDrawBuf, 0, ptCaret, bFLIP_SCREEN);
	...
	// Arab Ver.
	TextToPoint( strDrawBuf, -1, ptText, bFLIP_SCREEN);
	...
	if( m_strPrvText != m_strText ||
		...
		m_strPrvOuputText.GetLength() != strDrawBuf.GetLength() ||
...
void TEdit::OnKeyDown( UINT nChar, int nRepCnt, UINT nFlags)
{
	...
	{
	case VK_LEFT	:
		nChar = (m_nCountryFlag == COUNTRY_FLAG_EGYPT) ? VK_RIGHT : VK_LEFT;
		break;
	case VK_RIGHT	:
		nChar = (m_nCountryFlag == COUNTRY_FLAG_EGYPT) ? VK_LEFT : VK_RIGHT;
		break;
	}
...
BOOL TEdit::CheckCountryCharCode(  TCHAR chCharCode  )
{
	...
	// Arab Ver.
	else if ( m_nCountryFlag == COUNTRY_FLAG_EGYPT )
	{
		unsigned char unchCharCode = chCharCode;

		if( (unchCharCode == ' ') ||
			(vCHAR_DIR[unchCharCode] != CHAR_EXCEPTION))
		{
			return TRUE;
		}
