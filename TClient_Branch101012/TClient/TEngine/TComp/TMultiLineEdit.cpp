	  ...
	  m_nCUDLinePos(0), m_bSetupCUD(FALSE), m_nWheelMoveValue(2)
{
	...
	while( !EndOfKids(itr) )
	{
		...
		if( pComp->IsTypeOf(TCML_TYPE_SCROLL) )
		{
			...
			// Arab Ver.
			m_nScrollPos = (m_nCountryFlag == COUNTRY_FLAG_EGYPT) ? m_pScroll->m_rc.Width() : m_rc.Width() - pt.x;
...
void TMultiLineEdit::LocateCaret(INT nCaret)
{
	...
	for(INT nLine=0; nLine<nLINECNT; ++nLine)
	{
		...
		if( iStart <= nCaret && iEnd >= nCaret )
		{
			...
			// Arab Ver.
			strCurrentTextLine = m_vTextLine[nLine];
			...
			else
			{
				...
				if( m_nCtPosInLine > 0 )
				{
					...
					{
						CString strTextLeft = strCurrentTextLine;
						CSize sizeText;
						strTextLeft = GetCaretPoint(strCurrentTextLine, m_nCtPosInLine);
						GetTextExtentPoint(strTextLeft, sizeText);
						ptCaret.x += sizeText.cx;
					}
					else
