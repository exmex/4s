...
// Arab Ver.
BYTE CD3DFont::m_bStringFormat = 0;
						 ...
						 UINT nFormat)
{
	...
	if( m_nFormat == (nFormat&~DT_CALCRECT) &&
		m_strTEXT.GetLength() == strText.GetLength() &&
		!memcmp( LPCSTR(m_strTEXT), LPCSTR(strText), m_strTEXT.GetLength()) )
		return;
	...
	{
		// Arab Ver.
		StringFormat vFORMAT(m_bStringFormat);
		Graphics vGDI(hDC);
		// Arab Ver.
		if(CTachyonRes::m_bShowbar)
			vFORMAT.SetFormatFlags(StringFormatFlagsDisplayFormatControl);
