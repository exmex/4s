...
void CEXTMAPBar::BuildHEIGHT( LPEMBPARAM pParam)
{
	...
	if(pParam->m_nType)
	{
		...
		for( DWORD i=0; i<m_pItem->m_HugeMAP.m_nUnitCountZ; i++)
			for( DWORD j=0; j<m_pItem->m_HugeMAP.m_nUnitCountX; j++)
			{
				...
				m_pItem->UpdateUNIT( NULL, NULL, NULL, NULL, NULL, NULL, pHEIGHT, dwIndex);
	...
	else
	{
		...
		if( nIndex != -1 )
		{
			...
			m_pItem->UpdateUNIT( NULL, NULL, NULL, NULL, NULL, NULL, pHEIGHT, nIndex);
...
void CEXTMAPBar::SaveMinimap( LPEMBPARAM pParam)
{
	...
	if(pUNIT)
	{
		...
		vCAM.InitOrthoCamera(
			CMainFrame::m_3DDevice.m_pDevice,
			-10.0f,
			m_pItem->m_fMaxHeight + 500.0f/*m_pItem->m_fMaxHeight - (m_pItem->m_fMinHeight - 200.0f)*/,
		...
		for(int i = 0 ; i < nUnitLength ; i++)
		{
			for(int j = 0 ; j < nUnitLength ; j++)
			{
				...
				vSCALE._11 = m_pItem->m_vDETAILS[ pUNIT->m_pDETAIL[ i * (nUnitLength) + j ] ];
				vSCALE._33 = m_pItem->m_vDETAILS[ pUNIT->m_pDETAIL[ i * (nUnitLength) + j ] ];

				CMainFrame::m_3DDevice.m_pDevice->SetTransform( D3DTS_TEXTURE0, &vSCALE);
				CTextureItem *pDETAIL = CItemBase::GetTextureItem( m_pItem->m_vDETAIL[ pUNIT->m_pDETAIL[ i * (nUnitLength) + j ] ] );
		...
		CMainFrame::m_3DDevice.m_pDevice->SetRenderTarget( 0, pBACK);
		CMainFrame::m_3DDevice.m_pDevice->SetViewport(&vOLD);
		CMainFrame::m_3DDevice.m_pDevice->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &pBACK);
