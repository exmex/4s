...
BOOL CTChatShopDlg::OnInitDialog()
{
	...
	m_cShopList.SetExtendedStyle(
		LVS_EX_FULLROWSELECT|
		LVS_EX_GRIDLINES);
	m_cInvenList.InsertColumn( 0, _T("인벤토리"), LVCFMT_CENTER, rect.Width());

	m_cItemList.GetClientRect(&rect);
	rect.DeflateRect( 1, 1);
	m_cItemList.InsertColumn( 0, _T("이름"), LVCFMT_CENTER, rect.Width() * 2 / 3);
	m_cItemList.InsertColumn( 1, _T("상점가"), LVCFMT_CENTER, rect.Width() / 3);
	m_cShopList.InsertColumn( 0, _T("이름"), LVCFMT_CENTER, rect.Width() * 2 / 3);
	m_cShopList.InsertColumn( 1, _T("가격"), LVCFMT_CENTER, rect.Width() / 3);
