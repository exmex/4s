...
void CTWorldmapDlg::OnLButtonDown( UINT nFlags, CPoint pt)
{
	...
	if( IsVisible() && !TFrame::HitTest(pt) )
	{
		...
		{
			D3DXVECTOR2 vPOS(
				FLOAT(pt.x) - FLOAT(m_pDevice->m_option.m_dwScreenX / 2),
				FLOAT(m_pDevice->m_option.m_dwScreenY / 2) - FLOAT(pt.y));
			FLOAT fLength = ((FLOAT) m_pTMAP->m_pMAP->m_nUnitLength * m_pTMAP->m_pMAP->m_nTileLength);

			vPOS = m_vTCENTER + fLength * vPOS / (m_fTSCALE * FLOAT(TWORLDMAP_SIZE));
			m_pHost->m_vWorld._41 += vPOS.x - m_pHost->GetPositionX();
			m_pHost->m_vWorld._43 += vPOS.y - m_pHost->GetPositionZ();
			m_pHost->SetPositionX(vPOS.x);
			m_pHost->SetPositionZ(vPOS.y);
		}
#endif
		else
		{
