...
// --------------------------------------------------------------------
void CTGuildCommander::RequestGuildTacticsKickOut(const CString& strTarget, DWORD dwTarget, BOOL bWarn)
{
	CString strKickTarget;
	DWORD dwKickTarget;
...
// ====================================================================
void CTGuildCommander::RecvGuildPeer(BYTE bResult, const CString& strTarget, BYTE bPeer)
{
	CTClientWnd* pWnd = CTClientWnd::GetInstance();
	CTClientGame* pGame = CTClientGame::GetInstance();
	if( !pWnd || !pGame )
	...
	{
		BOOL bDutyChange = (strTarget == pMainChar->m_strNAME);
		CString from = GetPeerageStr(pMainChar->m_bGuildPeer);
...
void CTGuildCommander::RequestGuildDonationExp(DWORD dwEXP)
{
	...
	pSession->SendCS_GUILDCONTRIBUTION_REQ(dwEXP,0,0,0);
...
void CTGuildCommander::RequestGuildDonationMoney(DWORD dwRune,DWORD dwLuna,DWORD dwCron)
{
	...
	pSession->SendCS_GUILDCONTRIBUTION_REQ(0,dwRune,dwLuna,dwCron);
...
bool _compare_guildmember(const CTGuildCommander::GuildMember& l, const CTGuildCommander::GuildMember& r)
{
	...
	if( l.m_bService != r.m_bService )
		return l.m_bService > r.m_bService;
