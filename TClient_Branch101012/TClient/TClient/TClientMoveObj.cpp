								  ...
								  FLOAT fPosZ)
{
	...
	if( CTClientGame::GetInstance()->m_bTournamentBattle || CTClientGame::GetInstance()->m_bTournamentWatching )
		return 7.0f;
							   ...
							   WORD wMonID)
{
	...
	if(pTMON)
	{
		...
		InitSIZE(
			pTMON->m_fScaleX * GetAttrFLOAT(ID_SIZE_X),
			pTMON->m_fScaleY * GetAttrFLOAT(ID_SIZE_Y),
			pTMON->m_fScaleZ * GetAttrFLOAT(ID_SIZE_Z),
			0.0f, 0.0f);
	...
	while(it != pSFX->end())
	{
		...
		if( !pTSFX->m_pSFX || !pTSFX->IsDead() )
		{
			...
			if(pItem->m_pSFX->m_nPivotID)
				...
				pTSFX->m_vGlobal = m_vScaleRACE * pTSFX->m_vGlobal * m_vScaleSFX * m_vPosition;
			else
