...
{
	static const DWORD dwIconID[TDETAILINFO_ICON_COUNT+1] =
	{
		ID_CTRLINST_ITEM,		/// 아이템 아이콘
		ID_CTRLINST_SKILL,		/// 스킬 아이콘
		// Arab Ver.
		ID_CTRLINST_NAME		///텍스트 박스
	};

	static WORD nNewCnt = 0;
	m_id = MAKELONG(m_id, ++nNewCnt);
	// Arab Ver.
	for( BYTE i=0; i<(TDETAILINFO_ICON_COUNT+1); i++)
		m_vICON[i] = static_cast<TImageList*>( FindKid(dwIconID[i]) );

	CPoint pt;
	m_vICON[0]->GetComponentPos(&pt);
	// Arab Ver.
	int nIconWidth = m_vICON[0]->m_rc.Width();
	int nTextBoxWidth = m_vICON[2]->m_rc.Width();
	m_nTitlePosX_NIC = CTNationOption::FLIP_SCREEN ? pt.x + nIconWidth - nTextBoxWidth : pt.x;
