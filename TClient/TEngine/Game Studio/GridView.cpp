	...
	case VIEWPORT_FRONT	:
		{
			...
			nIndex = 0;
			for(FLOAT i=0.0f; i>=vMinPoint.x; i-=m_fGrid)
			{
				if( i <= vMaxPoint.x )
				{
					DrawLine(
			...
			nIndex = 0;
			for(FLOAT i=0.0f; i<=vMaxPoint.y; i+=m_fGrid)
			{
				if( i >= vMinPoint.y )
				{
					DrawLine(
			...
			nIndex = 0;
			for(FLOAT i=0.0f; i>=vMinPoint.y; i-=m_fGrid)
			{
				if( i <= vMaxPoint.y )
				{
					DrawLine(
	...
	case VIEWPORT_LEFT	:
		{
			...
			nIndex = 0;
			for(FLOAT i=0.0f; i>=vMinPoint.z; i-=m_fGrid)
			{
				if( i <= vMaxPoint.z )
				{
					DrawLine(
