	...
	case T3DFVF_WMESHVERTEX	:
		{
			...
			if(!vDATA.empty())
			{
				...
				for(int i=0; i<m_Mesh.m_pDATA->m_dwCount; i++)
				{
...
void CMeshItem::AddLevel( DWORD dwLevel)
{
	...
	for( int i=0; i<m_Mesh.m_dwMeshCount; i++)
	{
		...
		if( pINFO->m_dwCount > 0 )
		{
			...
			for(int j=0; j<INT(pINFO->m_vIB.size()); j++)
			{
	...
	for(int i=0; i<dwCount; i++)
	{
		...
		if(bEnable)
		{
			...
			{	
				...
				for(size_t j=0; j<vSHADOW.size(); j++)
					vSHADOW[j]->Release();
				vSHADOW.clear();
			}
			else
			...
			if( dwNodeCount > 0 )
			{
				...
				for(int i=0; i<dwNodeCount; i++)
				{
					...
					if( m_vAnimation.m_pNode[nNODE].m_nPositionKeyCount > 0 )
					{
						...
						for(int j=0; j<m_vAnimation.m_pNode[nNODE].m_nPositionKeyCount; j++)
						{
					...
					if( m_vAnimation.m_pNode[nNODE].m_nRotationKeyCount > 0 )
					{
						...
						for(int j=0; j<m_vAnimation.m_pNode[nNODE].m_nRotationKeyCount; j++)
						{
							if( bTYPE == 1 )
							{
						...
						if( bTYPE != 1 )
							for(int j=m_vAnimation.m_pNode[nNODE].m_nRotationKeyCount - 1; j>0; j--)
							{
								D3DXQUATERNION vINV;
					...
					if( m_vAnimation.m_pNode[nNODE].m_nScaleKeyCount > 0 )
					{
						...
						for(int j=0; j<m_vAnimation.m_pNode[nNODE].m_nScaleKeyCount; j++)
						{
