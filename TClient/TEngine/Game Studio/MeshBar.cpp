...
void CMeshBar::SetItemData()
{
	...
	for( int i=0; i<m_dwMeshCount; i++)
	{
		...
		if( m_dwLevel && pDoc->m_pCheck[i] )
			for(int j=0; j<INT(m_pItem->m_Mesh.m_pMESH[i][m_dwLevel - 1]->m_vIB.size()); j++)
		...
		case T3DFVF_WMESHVERTEX	:
			{
				...
				for(DWORD i=0; i<m_pItem->m_Mesh.m_dwMeshCount; i++)
				{
				...
				for(int i=0; i<DWORD(vDATA.size()); i++)
				{
		...
		case T3DFVF_MESHVERTEX	:
			{
				...
				for(int i=0; i<m_pItem->m_Mesh.m_dwMeshCount; i++)
				{
				...
				//for(int i=0; i<m_pItem->m_Mesh.m_dwMeshCount; i++)
				for(int i=0; i<mesh.m_dwMeshCount; i++)
				{
					LPIBINFO pINFO = new IBINFO();
