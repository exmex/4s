...
#include "Resource.h"
#include ".\tmapviewergame.h"

LPDIRECT3DVERTEXBUFFER9 CTMapViewerGame::m_pTRECTVB = NULL;

MAPTSKYBOX CTMapViewerGame::m_mapTSKYBOX;
MAPTLIGHT CTMapViewerGame::m_mapTLIGHT;
MAPTUNITFOG CTMapViewerGame::m_mapTFOG;


CString CTMapViewerGame::m_strTCMD[TCMD_COUNT] = {
	CString("EXIT"),		// TCMD_EXIT
	CString("LOADCHAR"),	// TCMD_LOADCHAR
	...
	CString("C"),			// TCMD_CLIMB
	CString("FR"),			// TCMD_FOGRANGE
	CString("FC"),			// TCMD_FOGCOLOR
	CString("FD")			// TCMD_FOGDENSITY
};

struct binary_draw : public binary_function< CTMapViewerObjBase *, CTMapViewerObjBase *, bool>
{
	bool operator () ( const CTMapViewerObjBase * &_Left, const CTMapViewerObjBase * &_Right) const
	{
...
CTMapViewerGame::CTMapViewerGame()
: m_bClimb(FALSE)
{
	m_pTCHATEDIT = NULL;
	m_pTCHAT = NULL;
...
LPTSKYBOX CTMapViewerGame::FindTSKYBOX( BYTE bUnitCountX,
									   BYTE bIndexID,
									   WORD wMapID,
									   BYTE bUnitX,
									   BYTE bUnitZ)
{
	MAPTSKYBOX::iterator finder = m_mapTSKYBOX.find(MAKELONG( MAKEWORD( bUnitZ * bUnitCountX + bUnitX, bIndexID), wMapID));

	if( finder != m_mapTSKYBOX.end() )
		return (*finder).second;

	return NULL;
}

LPTLIGHT CTMapViewerGame::FindTLIGHT( BYTE bUnitCountX,
									 BYTE bSetID,
									 BYTE bIndexID,
									 WORD wMapID,
									 BYTE bUnitX,
									 BYTE bUnitZ)
{
	MAPTLIGHT::iterator finder = m_mapTLIGHT.find(MAKELONG( MAKEWORD( bUnitZ * bUnitCountX + bUnitX, bSetID * TLIGHT_COUNT + bIndexID), wMapID));

	if( finder != m_mapTLIGHT.end() )
		return (*finder).second;

	return NULL;
}

void CTMapViewerGame::SetTLIGHT(BYTE bUnitCountX, 
							   BYTE bSetID, 
							   BYTE bIndexID, 
							   WORD wMapID,
							   BYTE bUnitX, 
							   ...
							   LPTLIGHT pLIGHT)
{
	...
	if( finder != m_mapTLIGHT.end() )
		(*finder).second = pLIGHT;
}

LPTUNITFOG CTMapViewerGame::FindTFOG( DWORD dwFogID)
{
	MAPTUNITFOG::iterator finder = m_mapTFOG.find(dwFogID);

	if( finder != m_mapTFOG.end() )
		return (*finder).second;

	return NULL;
}

LPTUNITFOG CTMapViewerGame::FindTFOG( WORD wMapID,
									 BYTE bUnitX,
									 BYTE bUnitZ)
{
	return FindTFOG(MAKELONG( MAKEWORD( bUnitX, bUnitZ), wMapID));
}


void CTMapViewerGame::SetTFOG(LPTUNITFOG pFOG, DWORD dwFogID)
{
	MAPTUNITFOG::iterator finder = m_mapTFOG.find(dwFogID);
...
void CTMapViewerGame::CalcSCENE( DWORD dwTick)
{
	D3DXVECTOR3 vPOS(
...
HRESULT CTMapViewerGame::Render( DWORD dwTickCount)
{
	if(m_vMAP.m_pMAP)
	{
...
void CTMapViewerGame::ResetTSKY()
{
	...
	if(pTFAR)
	{
		m_vFARIMAGE[m_bTSKY].InitOBJ(m_pRESMAP->GetOBJ(pTFAR->m_dwOBJ));
	...
	if(pTSKY)
	{
		m_vSKY[m_bTSKY].InitOBJ(m_pRESMAP->GetOBJ(pTSKY->m_dwOBJ));
...
void CTMapViewerGame::ResetTFOG( BYTE bWATER)
{
	FLOAT fUnitLength = FLOAT(m_vMAP.m_pMAP->m_nUnitLength * m_vMAP.m_pMAP->m_nTileLength);
...
void CTMapViewerGame::LerpTLIGHT( LPTLIGHT pTLIGHT,
								 BYTE bSetID,
								 BYTE bIndex,
								 BYTE bUnitX,
								 BYTE bUnitZ,
...
void CTMapViewerGame::LerpTFOG( LPTFOG pTFOG,
							   BYTE bUnitX,
							   BYTE bUnitZ,
							   FLOAT fTIME)
{
	LPTUNITFOG pTBLEND = FindTFOG(
		m_vMAP.m_wMapID,
		bUnitX,
		bUnitZ);
...
void CTMapViewerGame::ResetVisibleOBJ( DWORD dwTick)
{
	m_vMAP.ResetTMAP(
		m_pDevice,
		m_pCamera,
...
void CTMapViewerGame::PushDrawOBJ( CTMapViewerObjBase *pTOBJ)
{
...
void CTMapViewerGame::RenderTOBJ( LPLISTTOBJBASE pLIST)
{
	...
	/*CTMapViewerObjBase::BeginDrawSquare(m_pDevice);
	SetMaterial(NULL);
	CTMapViewerObjBase::m_pTSHADOW->m_dwCurTick = m_dwTSHADOW;

	UVKEY vKEY = CTMapViewerObjBase::m_pTSHADOW->GetUV();*/
	...
	m_pDevice->m_pDevice->SetTexture( 0, CTMapViewerObjBase::m_pTSHADOW->GetTexture());
...
void CTMapViewerGame::RenderTOBJ( CTMapViewerObjBase *pTOBJ)
{
...
void CTMapViewerGame::RenderTPATH()
{
	for( int i=0; i<INT(m_vMAP.m_vTPATH.size()); i++)
		m_vMAP.m_vTPATH[i]->Render(m_pDevice);
}

void CTMapViewerGame::RenderTSILHOUETTE()
{
	CTMapViewerCAM vTCAM;

	vTCAM.InitCamera(
		m_pDevice->m_pDevice,
		m_pCamera->m_fFarPlane - m_pCamera->m_fNearPlane,
...
void CTMapViewerGame::RenderTTEXT( LPLISTTOBJBASE pLIST)
{
	LISTTOBJBASE::iterator itTOBJ;
	D3DXMATRIX vWORLD;
...
void CTMapViewerGame::RenderTTEXT( LPD3DXMATRIX pWORLD, CTMapViewerObjBase *pTOBJ)
{
	FLOAT fLength = D3DXVec2Length(&D3DXVECTOR2(
...
void CTMapViewerGame::RenderTSQUARE( CTMapViewerObjBase *pTOBJ,
								    LPD3DXMATRIX pUV,
									LPD3DXVECTOR2 pDIR,
									FLOAT fSCALE)
{
	...
	else
	{
		...
		m_pDevice->m_pDevice->SetStreamSource( 0, CTMapViewerObjBase::m_pTSQUARE, 0, sizeof(PVERTEX));
		m_pDevice->m_pDevice->SetTransform( D3DTS_WORLD, &vMAT);
		m_pDevice->m_pDevice->DrawPrimitive( D3DPT_TRIANGLESTRIP, 0, 2);
	}
}

void CTMapViewerGame::CalcClickMove( DWORD dwTick)
{
	D3DXVECTOR3 vPOS(
		m_pMainChar->m_vPosition._41,
		m_pMainChar->m_vPosition._42,
...
void CTMapViewerGame::CalcMainChar( DWORD dwTick)
{
...
FLOAT CTMapViewerGame::CalcCamera( DWORD dwTick)
{
	int nCamRotX = 0;
	int nCamRotY = 0;
	int nCamZoom = 0;
...
BYTE CTMapViewerGame::FixCamera( DWORD dwTick)
{
...
FLOAT CTMapViewerGame::CheckCamPos()
{
...
void CTMapViewerGame::RotCameraX( int nCamRotX)
{
...
void CTMapViewerGame::RotCameraY( int nCamRotY)
{
...
void CTMapViewerGame::OnCommand( DWORD cmd)
{
	int nERROR = FALSE;
...
void CTMapViewerGame::OnCommand( DWORD dwCMD, CString strParam)
{
	...
	switch(dwCMD)
	{
	ON_MAPVIEWER_CMD(TCMD_MATERIALA)
	ON_MAPVIEWER_CMD(TCMD_MATERIALD)
	ON_MAPVIEWER_CMD(TCMD_LOADCHAR)
	ON_MAPVIEWER_CMD(TCMD_MAPCOLOR)
	ON_MAPVIEWER_CMD(TCMD_LOADMAP)
	ON_MAPVIEWER_CMD(TCMD_MINIMAP)
	ON_MAPVIEWER_CMD(TCMD_SHADOW)
	ON_MAPVIEWER_CMD(TCMD_SPEED)
	ON_MAPVIEWER_CMD(TCMD_LIGHT)
	ON_MAPVIEWER_CMD(TCMD_MOVE)
	ON_MAPVIEWER_CMD(TCMD_EXIT)
	ON_MAPVIEWER_CMD(TCMD_HELP)
	ON_MAPVIEWER_CMD(TCMD_FPS)
	ON_MAPVIEWER_CMD(TCMD_POS)
	ON_MAPVIEWER_CMD(TCMD_CLIMB)
	ON_MAPVIEWER_CMD(TCMD_FOGRANGE)
	ON_MAPVIEWER_CMD(TCMD_FOGCOLOR)
	ON_MAPVIEWER_CMD(TCMD_FOGDENSITY)

	ON_MAPVIEWER_CMD(TCMD_LANDCOLLISION)
	ON_MAPVIEWER_CMD(TCMD_LIGHTMODE)
	ON_MAPVIEWER_CMD(TCMD_LIGHTDIR)
	ON_MAPVIEWER_CMD(TCMD_LIGHTA)
	ON_MAPVIEWER_CMD(TCMD_LIGHTD)
	}
...
void CTMapViewerGame::OnERROR( DWORD dwERROR)
{
	CString strTCMD;
	CString strTYPE;
	CString strTMSG;
	...
	case TERROR_FOGRANGE			: strTCMD = m_strTCMD[TCMD_FOGRANGE]; strTMSG.LoadString(IDS_ERROR_FOGRANGE); break;
	case TERROR_FOGCOLOR			: strTCMD = m_strTCMD[TCMD_FOGCOLOR]; strTMSG.LoadString(IDS_ERROR_FOGCOLOR); break;
	case TERROR_FOGDENSITY			: strTCMD = m_strTCMD[TCMD_FOGDENSITY]; strTMSG.LoadString(TERROR_FOGDENSITY); break;
	}

	ChatMSG( strTYPE, strTCMD, strTMSG, 0xFFFFFFF0);
}

void CTMapViewerGame::ReleaseGame()
{
	m_bLButtonDown = FALSE;
	m_bRButtonDown = FALSE;
...
void CTMapViewerGame::OnActivateKEY( BYTE bSCAN)
{
	...
	case TKEY_FORWARD			:
	case TKEY_BACK				: m_bAutoRun = FALSE;
	default						: m_bMoveType = TMAINMOVE_NORMAL; break;
	}
}

void CTMapViewerGame::OnReleaseKEY( BYTE bSCAN)
{
...
void CTMapViewerGame::OnRegionChanged()
{
	m_dwActiveVolume = TBGM_VOLUME;
	m_bStopBGM = TRUE;
}

void CTMapViewerGame::OnImeComposition( HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	CFrameGroup::OnImeComposition( hWnd, wParam, lParam);
}

void CTMapViewerGame::OnChar( UINT nChar, int nRepCnt, UINT nFlags)
{
...
void CTMapViewerGame::OnKeyDown( UINT nChar, int nRepCnt, UINT nFlags)
{
	if(!m_bChatON)
...
void CTMapViewerGame::OnKeyUp( UINT nChar, int nRepCnt, UINT nFlags)
{
	if(!m_bChatON)
...
void CTMapViewerGame::OnMouseWheel( UINT nFlags, short zDelta, CPoint pt)
{
	m_nCamZOOM -= zDelta;
	CFrameGroup::OnMouseWheel( nFlags, zDelta, pt);
}

void CTMapViewerGame::OnMouseMove( UINT nFlags, CPoint pt)
{
	CPoint point(
...
void CTMapViewerGame::OnLButtonDown( UINT nFlags, CPoint pt)
{
...
void CTMapViewerGame::OnLButtonDblClk( UINT nFlags, CPoint pt)
{
	CFrameGroup::OnLButtonDblClk( nFlags, pt);
}

void CTMapViewerGame::OnRButtonDown( UINT nFlags, CPoint pt)
{
...
void CTMapViewerGame::OnLButtonUp( UINT nFlags, CPoint pt)
{
...
void CTMapViewerGame::OnRButtonUp( UINT nFlags, CPoint pt)
{
...
void CTMapViewerGame::OnClick( UINT nFlags, CPoint pt)
{
	if(!HitTest(pt))
	{
...
void CTMapViewerGame::OnRClick( UINT nFlags, CPoint pt)
{
}

void CTMapViewerGame::SetMaterial( CTMapViewerObjBase *pTOBJ)
{
...
void CTMapViewerGame::SetAmbient( BYTE bRValue,
								 BYTE bGValue,
								 BYTE bBValue)
{
	m_pDevice->m_pDevice->SetRenderState(
...
void CTMapViewerGame::EnableLIGHT( CTMapViewerBSP *pTBSP)
{
...
void CTMapViewerGame::EnableLIGHT()
{
...
void CTMapViewerGame::DisableLIGHT()
{
...
void CTMapViewerGame::ResetSCENEConstant()
{
...
LRESULT CTMapViewerGame::OnOBJMsg( LPOBJMSG pMSG)
{
	switch(pMSG->m_dwMSG)
	{
	case OM_TSUBACTENDING	:
		{
			CTMapViewerObjBase *pTOBJ = (CTMapViewerObjBase *) pMSG->m_pOBJ;
	...
	case OM_ACTENDING		:
		{
			...
			if( pTOBJ->m_bACTLevel != CTMapViewerObjBase::m_vTACTION[pTOBJ->m_bAction]->m_bLevel )
			{
				pTOBJ->m_bACTLevel = CTMapViewerObjBase::m_vTACTION[pTOBJ->m_bAction]->m_bLevel;
				return 1;
			}
		}
...
void CTMapViewerGame::SwitchFocus( TComponent *pCandidate)
{
...
void CTMapViewerGame::EnableChat( BYTE bEnable)
{
...
CString CTMapViewerGame::ChatMSG( CString strTYPE,
								 CString strNAME,
								 CString strMSG,
								 DWORD dwCOLOR)
{
	...
	while(!strTEXT.IsEmpty())
	{
		...
		while(!strTEXT.IsEmpty())
		{
			...
			while( szTEXT.cx > szITEM.cx )
			{
				...
				TCHAR nCode = strFORMAT.GetAt(nLength);

				strFORMAT.Delete(nLength);
				nLength--;

				if(IsDBCSLeadByte(nCode))
					strFORMAT.Delete(nLength);
...
void CTMapViewerGame::ErrorMSG( DWORD dwTypeID,
							   DWORD dwTitleID,
							   DWORD dwMsgID,
							   DWORD dwCOLOR)
{
...
void CTMapViewerGame::CalcBGM( DWORD dwTick)
{
	...
	if(m_bStopBGM)
	{
		...
		if(m_dwActiveVolume)
		{
			CTShow *pDSHOW = (CTShow *) CTachyonRes::m_MEDIA.GetData(
				MEDIA_DSHOW,
				m_dwBGM);
			BYTE bMasterVolume = CTShow::m_bMasterVolume;

			CTShow::m_bMasterVolume = BYTE(CTShow::m_bMasterVolume * m_dwActiveVolume / TBGM_VOLUME);
			pDSHOW->ResetVolume();
			CTShow::m_bMasterVolume = bMasterVolume;
		}
		else
		{
	...
	else if(!CTachyonRes::m_MEDIA.IsPlay( MEDIA_DSHOW, m_dwBGM, 0))
	{
		dwBGMTick += dwTick;

		if( dwBGMTick > TBGM_TIMER )
		{
			if( m_pMainChar->m_pTREGION && !m_pMainChar->m_pTREGION->m_vTBGM.empty() && !(rand() % TBGM_RATE) )
			{
				CTachyonRes::m_MEDIA.Stop(
					MEDIA_DSHOW,
					m_dwBGM, 0);
				m_dwBGM = m_pMainChar->m_pTREGION->m_vTBGM[rand() % INT(m_pMainChar->m_pTREGION->m_vTBGM.size())];
...
void CTMapViewerGame::CalcGlobalTEX( DWORD dwTick)
{
	if(CTMapViewerObjBase::m_pTSHADOW)
	{
		m_dwTSHADOW += dwTick;
		m_dwTSHADOW %= CTMapViewerObjBase::m_pTSHADOW->m_dwTotalTick;
	}
}

void CTMapViewerGame::CalcTSKY( FLOAT fWaterHeight,
							   DWORD dwTick)
{
...
void CTMapViewerGame::ParseCMD( CString strCMD)
{
	int nPOS = 0;
...
void CTMapViewerGame::DeleteFlag()
{
...
void CTMapViewerGame::AddFlag()
{
	...
	if(m_vMAP.m_pMAP->m_pENABLE[bUnitZ * m_vMAP.m_pMAP->m_nUnitCountX + bUnitX])
	{
		...
		pTFLAG->m_vNAME.SetFont(CTMapViewerObjBase::m_pFONT);
