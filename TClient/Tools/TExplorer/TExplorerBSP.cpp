...
MAPTBSPMAP CTExplorerBSP::m_mapTBSPMAP;

CTExplorerBSP::CTExplorerBSP()
{
...
CTachyonBSPMAP *CTExplorerBSP::FindTBSPMAP( DWORD dwMapID)
{
...
void CTExplorerBSP::ReleaseTBSPMAP()
{
...
void CTExplorerBSP::InitTBSPMAP()
{
	ReleaseTBSPMAP();
}

BYTE CTExplorerBSP::Lock( CTachyonRes *pTRES,
						  DWORD dwBspID)
{
	if( !pTRES || !dwBspID )
		return FALSE;
...
void CTExplorerBSP::InitTBSP( LPMAPOBJ pTOBJ)
{
...
void CTExplorerBSP::ResetTLIGHT()
{
	if(!m_pMAP)
		return;
...
void CTExplorerBSP::ResetPVS( DWORD dwNodeID)
{
	if( !m_pINFO || !m_pMAP )
		return;
...
DWORD CTExplorerBSP::GetNodeID( D3DXVECTOR3 vPOS)
{
	if( !m_pINFO || !m_pMAP )
		return NODEID_NULL;
