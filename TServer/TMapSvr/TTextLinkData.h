...
protected:
	...
	LPQUESTTEMP		m_pLastUnpackQuest;	
	CTItem*			m_pLastUnpackItem;
...
public:
	...
	void PackQuest(LPQUESTTEMP pQuest);
	LPQUESTTEMP UnPackQuest();

	void PackItem(CTItem* pClientItem);
	CTItem* UnPackItem();

	LPQUESTTEMP GetLastUnkpackQuest() const		{ return m_pLastUnpackQuest; }
	CTItem* GetLastUnkpackItem() const			{ return m_pLastUnpackItem; }
	void ResetPosition()						{ m_nPos = 0; }
	BOOL IsEndOfPosition() const				{ return ((UINT)m_vData.GetLength() <= m_nPos); }
