...
void CStoryDlg::InitWeb()
{
	...
	strNUM.Format( _T("?s=%d"), nKEY);
	if(strUrl == _T(""))
		strUrl = LAUNCHERWEB_NAME;

	m_webCtrl.Create( _T("WebControl"), WS_CHILD|WS_VISIBLE, WebRect, (CWnd *) this, 1001);
	m_webCtrl.Navigate( strUrl + strNUM, NULL, NULL, NULL, NULL);
	m_webCtrl.MoveWindow(rect.Width()/3-30, 92, rect.Width()/2+137, rect.Height()/2+46);
	
	//m_webSearch.Create( _T("Search"), WS_CHILD|WS_VISIBLE, CRect( -100, -100, -10, -10), (CWnd *) this, 1002);
	//switch(nKEY)
	//{
	//case 0	:
	//case 1	: m_webSearch.Navigate( CString(SEARCHN_EXEC1), NULL, NULL, NULL, NULL); break;
	//case 2	:
	//case 3	: m_webSearch.Navigate( CString(SEARCHN_EXEC2), NULL, NULL, NULL, NULL); break;
	//case 4	:
	//case 5	: m_webSearch.Navigate( CString(SEARCHN_EXEC3), NULL, NULL, NULL, NULL); break;
	//case 6	: m_webSearch.Navigate( CString(SEARCHD_EXEC1), NULL, NULL, NULL, NULL); break;
	//default : m_webSearch.Navigate( CString(SEARCHD_EXEC2), NULL, NULL, NULL, NULL); break;
	//}
}
...
BOOL CStoryDlg::ReadRegistry()
{
	...
	m_bReadRegistry = TRUE;

	return TRUE;
}

BOOL CStoryDlg::ReadIsFirst()
{
	HKEY hKeyRet;	
	...
	cbdata = 1024;
	memset(data, 0, 1024);	
	err = RegQueryValueEx(hKeyRet, _T("IsFirst"), NULL, &type, data, &cbdata);

	if( ERROR_SUCCESS != err || type != REG_DWORD)
		theApp.m_IsFirst = TRUE;
	else
		theApp.m_IsFirst = *((LPDWORD)data);
...
void CStoryDlg::CheckIsFirstExe()
{
	if( theApp.m_IsFirst )
	{
		SettingDoModal();
		...
		m_pdlgPlaySetting->ShowGameSetting();
	}
}
