...
void CGameSetting::OnPaint()
{
	...
	// 그리콅E메시지에 큱E漫??CDialog::OnPaint()을(를) 호출하햨E마십시오.
}
...
BOOL CGameSetting::OnInitDialog()
{
	...
	if(!ReadRegistry())
	{
		//MessageBox(_T("Can not read Registry"),MSGBOX_TITLE,MB_OK);
	}
...
{
	// 다이얼로그 배컖E비트맵 투뫄?E적퓖E
	COLORREF crTransparent = _color;	// 투뫄?E 검정(0,0,0)	
	BITMAP Bitmap;	
	...
	if(m_strCountry == _T("GERMANY") )
	{
		...
		case OPTION_LOW: // Low
			{				
				...
				m_dwObjDetailValue		= 1; // 캐릭터 품햨E
				m_dwMapDetailValue		= 0; // 지?E품햨E
				m_dwTextureDetailValue	= 0; // 텍스쳐 품햨E
			}
			break;
		case OPTION_MID: // Mid
			{
				...
				m_dwObjDetailValue		= 2; // 캐릭터 품햨E
				m_dwMapDetailValue		= 1; // 지?E품햨E
				m_dwTextureDetailValue  = 1; // 텍스쳐 품햨E
		...
		case OPTION_HI: // High
			{
				...
				m_dwObjDetailValue		= 2; // 캐릭터 품햨E
				m_dwMapDetailValue		= 1; // 지?E품햨E
				m_dwTextureDetailValue  = 2; // 텍스쳐 품햨E
			}
			break;
		}
	}
