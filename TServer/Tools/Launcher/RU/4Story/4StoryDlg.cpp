	...
	case WM_RESTART_PATCH:
		{
			...
			if(!CreateProcess(strNew, NULL, 
				...
				AfxMessageBox(STRING_DOWNLOAD_FAIL);
		}
		break;
...
void CStoryDlg::OnConnect(int nErrorCode)
{
	if(nErrorCode)
	{
		AfxMessageBox(STRING_CONNECT_FAIL);
		return;
	}
	else
		SendCT_PATCH_REQ();
...
DWORD CStoryDlg::OnCT_PATCH_ACK(CPacket * pPacket)
{
	...
	if(wCount)
	{
		...
		if( !m_hDownload )
		{
			AfxMessageBox(STRING_THREAD_FAIL);
			return 1;
		}
...
BYTE CStoryDlg::BeginProtect()
{
	...
	if( !m_vModuleGuard.InitProtector( MAKEINTRESOURCE( IDR_TCLIENTMP1 ), _T("TClientMP") ) ||
...
//	return vLauncher.ExecPE( &CString(_T(".\\TClient.exe")), &strCommandLine);
