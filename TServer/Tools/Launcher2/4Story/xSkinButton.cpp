...
void CxSkinButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	{	// DISABLED BUTTON
		if(m_bDisabled.IsNull()) // no skin selected for disabled state -> standard button
	...
	{	// SELECTED (DOWN) BUTTON
		...
		{
			if(m_bDown.IsNull()) // no skin selected for selected state -> standard button
				pDC->FillSolidRect(&r,GetSysColor(COLOR_BTNFACE));
            else // paint the skin
		...
		{	
			if(m_bNormal.IsNull())	 // no skin selected for normal state -> standard button
				pDC->FillSolidRect(&r,GetSysColor(COLOR_BTNFACE));
			else // paint the skin
			{
                if ((m_tracking)&&(!m_bOver.IsNull()))
					DrawBitmap(pDC,(HBITMAP)m_bOver,r,m_DrawMode);
				else
				{
					if ((lpDrawItemStruct->itemState & ODS_FOCUS)&&(!m_bFocus.IsNull()))
...
{
	m_bNormal.Destroy();	//free previous allocated bitmap
	m_bDown.Destroy();
	m_bOver.Destroy();
	m_bDisabled.Destroy();
	m_bMask.Destroy();
	...
	if (normal>0) m_bNormal.LoadFromResource(hInst,normal);
	if (down>0)	  m_bDown.LoadFromResource(hInst,down);
	if (over>0)	  m_bOver.LoadFromResource(hInst,over);
	if (focus>0)  m_bFocus.LoadFromResource(hInst,focus);

	if (disabled>0) m_bDisabled.LoadFromResource(hInst,disabled);
	else if (normal>0) m_bDisabled.LoadFromResource(hInst,normal);
	...
	if (mask>0){
		m_bMask.LoadFromResource(hInst,mask);
...
BYTE CxSkinButton::SetSkin(LPCTSTR normal,LPCTSTR down, LPCTSTR over, LPCTSTR disabled, LPCTSTR focus,LPCTSTR mask,
			 short drawmode,short border,short margin)
{
	m_bNormal.Destroy();	//free previous allocated bitmap
	m_bDown.Destroy();
	...
	if (normal>0) m_bNormal.Load(normal);
	if(m_bNormal.IsNull())
		return false;
	if (down>0)	  m_bDown.Load(down);
	if (over>0)	  m_bOver.Load(over);
	if (focus>0)  m_bFocus.Load(focus);

	if (disabled>0) m_bDisabled.Load(disabled);
	else if (normal>0) m_bDisabled.Load(normal);
...
/////////////////////////////////////////////////////////////////////////////
void CxSkinButton::SetToolTipText(LPCTSTR s)
{
	if(m_tooltip.m_hWnd==NULL){
		...
		m_tooltip.UpdateTipText(s,this);
	}
}
/////////////////////////////////////////////////////////////////////////////
